...
echo "We use $NODE_NUM nodes to train with ${TRAINDATANUM} samples for ${EPOCH} epochs, resulting in ${TOTALSTEP} running steps"
echo "We use step DPO strategy for training, thus we will save models when every epoch ends"

for ((i=1; i<=${EPOCH}; i++))
    do

    if [[ ${raw_model_path} == *"2.8b"* ]]
    then
        # split 1-epoch training of 2.8b into 3 epochs, using 1/3 data in each epoch
        train_data_path=dummy_$(($i-1)).jsonl
    fi
    ...
    then
        ...
        train_model_path=${model_output_path}epoch$(($i-1))/
    fi
    ...
    torchrun --nnodes=$NODE_NUM \
        ...
        --bf16 True \
        --output_dir ${model_output_path}epoch${i}/ \
        --num_train_epochs 1 \
